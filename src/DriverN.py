...
def getD2Alpha( TriList, edgeList ):
    d2 = []
    n = len(edgeList)

    for row in TriList:
        newTriangle = [0 for i in range(n)]
        newTriangle[edgeList.index( (row[0],row[1] ) )] = 1
        newTriangle[edgeList.index((row[0],row[2]) )] = 1
        newTriangle[edgeList.index( (row[1],row[2]) )] = 1
        d2.append(newTriangle)
...
def compute_cyclesN(GlobalOptions, epsList):
    ...
    if method == 'Alpha': # Alpha Complex
        
        Dist = sp.spatial.distance_matrix(W, W, p=2)
          
        for e in epsList:

            # now W is the point cloud
            
            # set the distance matrix 
            if not GlobalOptions['shortEdges']:
                
                TriList, EL, TriScales, EdgeScales = AlphaComplex( W , e)
                
                EL = [x.tolist() for x in EL]
                
                Wstep = np.zeros( Dist.shape )
                
                for jj,edge in enumerate(EL): # set distances 
                    Wstep[ edge[0],edge[1] ] = EdgeScales[jj]
                    Wstep[ edge[1],edge[0] ] = EdgeScales[jj]
                
                        
                maximal = SimplexGraph.getEdgeList(Wstep)

            
            else: # if short edges
                
                TriList, TriEdges = AlphaComplex( W , e, short=True)
                
                Wstep = np.zeros( Dist.shape )
                for e in TriEdges: # set distances
                    Wstep[ e[0],e[1] ] = Dist[e[0],e[1]]
                    Wstep[ e[1],e[0] ] = Dist[e[0],e[1]]
            ...
            if GlobalOptions['shortEdges']:
                
                # convert TriEdges to list of tuples
                
                d2 = getD2Alpha( TriList, [tuple(x.tolist()) for x in TriEdges] ) 
                
               
            else:
                #d2 = getD2( Wstep , maximal[0] )
                # Correct for the Alpha complex
                
                d2 = getD2Alpha( TriList, maximal[0] ) 
...
def getFiltrBasisN(W, epsList ,method = 'VR', shortEdges=False):
    """ Input a matrix W, the list of spatial thresholds, method = ['VR' | 'Alpha'], optionally the edge list 
        If method is VR the matrix is a weighted adjacency matrix. Othewise it is a point cloud. 
        Output a weighted adjacency matrix S that is the minimal homological scaffold of H_1
    """
    ...
    if method != 'VR' and method != 'Alpha':
        raise ValueError('Wrong method')

    NV = len(W)
    GlobalOptions = {}
    
    if method == 'Alpha' and shortEdges:
        
        GlobalOptions['shortEdges'] = True
        
    else:
        GlobalOptions['shortEdges'] = False
    ...
    GlobalOptions['N'] = NV
    GlobalOptions['EdgeList'] = maximal
    GlobalOptions['Method'] = method
    GlobalOptions['Draws'] = False
    GlobalOptions['parallel'] = False
    GlobalOptions['monitor'] = None
    GlobalOptions['ReturnMatrix'] = False
    GlobalOptions['ReturnMaximal'] = False

    #Filtr = compute_cyclesN(GlobalOptions, epsList)

    return compute_cyclesN(GlobalOptions, epsList)
